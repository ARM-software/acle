...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
* Support added for Function Multi Versioning.
...
These attributes have no effect on the calling convention, all versions
must use the same calling convertion.

Function Multi Versioning (FMV)
================================

Function Multi Versioning provides a convenient way to select the most
appropriate version of a function in runtime. All versions
of the function may be in the final binary. The compiler generates
all supported versions and the runtime makes the selection in load time.

The following attributes trigger the multi variant code generation:
``target_version``, ``target_clones``.
These attributes can't be mixed.
The "default" variant means the version of the function that would be
generated without these attributes.

This feature might be turned off in compile time. In this case the
"default" variant shall be used.
...
when applied to a function it will become one of the versions. Function with
the same name may exist with multiple versions in the same translation unit.
One "default" version of the  function is required to be provided.
Implicitly, when without attribute, or explicitly providing the "default" in
the attribute.
If only the "default" variant exist it should be linked directly.

::

  __attribute__((target_clones("name",...)))

when applied to a function the compiler emits multiple versions based
on the arguments. One of them is implicitly the "default". If a name is not
recognized the compiler should ignore it. If the default matches with an other
explicitly provided version the compiler can emit only one function instead of
the two.
...
Version to be mangled. <vendor-specific suffix> should hold the
version information [cxxabi]_ as:
...
.. table:: Architecture feature mapping for AArch32
   :widths: 10 20 11 30

   +--------------+-------------------------------------+---------------------+-------------------------------------+
   | **Priority** | **Architecture feature name**       | **Name**            | **Dependent feature registers**     |
   +==============+=====================================+=====================+=====================================+
   | 0            | N/A                                 | default             | N/A                                 |
   +--------------+-------------------------------------+---------------------+-------------------------------------+
...
.. table:: Architecture feature mapping for AArch64
   :widths: 10 20 11 30
   ...
   +--------------+-------------------------------------+---------------------+-------------------------------------+
   | 60           | ``FEAT_SM3`` && ``FEAT_SM4``        | sm                  | ID_AA64ISAR0_EL1.SM4 == 0b0001 &&   |
   ...
   +--------------+-------------------------------------+---------------------+-------------------------------------+
   | 250          | SVE                                 | sve                 | ID_AA64PFR0_EL1.SVE != 0b0000 &&    |
   |              |                                     |                     | ID_AA64ZFR0_EL1.SVEver == 0b0000    |
   +--------------+-------------------------------------+---------------------+-------------------------------------+
   | 260          | ``FEAT_BF16``                       | sve_bf16            | ID_AA64ZFR0_EL1.BF16 != 0b00000     |
   +--------------+-------------------------------------+---------------------+-------------------------------------+
   ...
   +--------------+-------------------------------------+---------------------+-------------------------------------+
   | 310          | ``FEAT_SVE2``                       | sve2                | ID_AA64PFR0_EL1.SVE != 0b0000 &&    |
   |              |                                     |                     | ID_AA64ZFR0_EL1.SVEver == 0b0001    |
   +--------------+-------------------------------------+---------------------+-------------------------------------+
   | 320          | ``FEAT_SVE_AES``                    | sve_aes             | ID_AA64ZFR0_EL1.AES == 0b0001 ||    |
   ...
   +--------------+-------------------------------------+---------------------+-------------------------------------+
   | 390          | ``FEAT_MTE3``                       | mte3                | ID_AA64PFR1_EL1.MTE >= 0b0010       |
   +--------------+-------------------------------------+---------------------+-------------------------------------+
...
Implementation of the selection algoritm is platform dependent.
The selection is permanent for the lifetime of the process.
Only those versions could be considered where all dependent features are
available.
